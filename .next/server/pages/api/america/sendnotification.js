...
async function sendnotification(req, res) {
    if (req.method === "POST") {
        ...
        await enviarPush();
        // console.log("TODOS LOS TICKET SON: ", tickets);
        ...
        tickets.forEach((el, i)=>{
            ...
            if (el.status == "error") {
                if (el.details.error == "DeviceNotRegistered") {
                    ...
                    let indexTicketError = i;
                    let idFireUser = usersArray[i];
                    let tokenPushUser = userFinal[idFireUser].token;
                    DiviceNoRegisterUserId.push(idFireUser);
                // console.log("USUARIO ERROR ES: ", indexTicketError, tokenPushUser, idFireUser);
                }
            }
        });
